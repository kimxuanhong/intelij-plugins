...
class GoImplementActionV2 : AnAction() {
    ...
    private fun generateImplCode(interfaceCode: String): String {
        ...
        val implName = interfaceName[0].lowercaseChar() + interfaceName.substring(1);
        val builder = StringBuilder()
        builder.append(interfaceCode)
        builder.append("\n\n")

        builder.append("type ").append(implName).append(" struct {\n")
        builder.append("}\n\n")

        builder.append("var instance").append(interfaceName).append(" *").append(implName).append("\n")
        val once = implName + "Once"
        builder.append("var ").append(once).append(" sync.Once").append("\n\n")

        builder.append("func New").append(interfaceName).append("() ").append(interfaceName).append(" {\n")
        builder.append("    ").append(once).append(".Do(func() {").append("\n")
        builder.append("        instance").append(interfaceName).append(" = &").append(implName).append("{}\n")
        builder.append("    })\n")
    ...
    private fun generateMethodImpl(methodSignature: String, interfaceName: String): String {
        ...
        val implName = interfaceName[0].lowercaseChar() + interfaceName.substring(1);
        method.append("func (r *").append(implName).append(") ").append(methodSignature.trim { it <= ' ' })
